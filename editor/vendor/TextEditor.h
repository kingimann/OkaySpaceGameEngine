...
public:
	...
	// --- OkaySpace additions: geometry accessors for host-drawn overlays ------
	// Screen position just below the caret (for an autocomplete popup), the row
	// height, and the monospace character width. Valid after Render() has run once.
	ImVec2 CaretScreenPosBelow() const {
		Coordinates c = GetActualCursorCoordinates();
		return ImVec2(mLastContentOrigin.x + mTextStart + TextDistanceToLineStart(c),
		              mLastContentOrigin.y + (c.mLine + 1) * mCharAdvance.y);
	}
...
private:
	...
	// OkaySpace: captured during Render() so the host can place overlays (autocomplete).
	ImVec2 mLastContentOrigin = ImVec2(0.0f, 0.0f);
	int    mLastFirstVisibleLine = 0;
};
